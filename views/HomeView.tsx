...
import { TrendingUp, TrendingDown, Wallet, Calendar, ChevronRight, Info, Lock, Eye, CheckCircle2, ShieldCheck, Image as ImageIcon, X } from 'lucide-react';
...
const HomeView: React.FC<HomeViewProps> = ({ stats, entries, years, selectedYearId, setSelectedYearId, isAdmin }) => {
  ...
  const handleUnlockCollections = (e: React.FormEvent) => {
    e.preventDefault();
    if (collectionPassword === 'admin123') {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="glass-card p-8 rounded-[2rem] shadow-xl space-y-6 relative overflow-hidden">
          ...
          {!effectiveUnlocked ? (
            <form onSubmit={handleUnlockCollections} className="space-y-4 pt-2">
              ...
              <button 
                type="submit"
                className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-xl shadow-emerald-200 hover:bg-emerald-700 hover:scale-[1.02] transition-all active:scale-95 flex items-center justify-center space-x-2"
              >
                <span>Access Secure Records</span>
                <ChevronRight className="w-5 h-5" />
              </button>
              {passwordError && <p className="text-center text-xs text-rose-500 font-bold uppercase tracking-widest">Verification Failed</p>}
            </form>
          ) : (
            <div className="pt-2">
              <button 
      ...
      <div className="p-6 bg-white/40 backdrop-blur-md rounded-[2rem] border-2 border-white/60 flex items-start space-x-4 shadow-lg">
        <div className="bg-indigo-100 p-2 rounded-xl text-indigo-600">
          <Info className="w-6 h-6 flex-shrink-0" />
        </div>
        <p className="text-sm text-slate-700 font-medium leading-relaxed pt-1">
          <strong>Transparency:</strong> All expenses are public. Click the image icons above to view receipt or proof of payment. For privacy, collection details are restricted to authorized personnel.
        </p>
      </div>

      {/* Image Modal */}
      {viewingImage && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setViewingImage(null)}>
          <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center" onClick={e => e.stopPropagation()}>
            <button 
              onClick={() => setViewingImage(null)}
              className="absolute -top-12 right-0 p-2 bg-white/10 text-white rounded-full hover:bg-white/20 transition-all"
            >
              <X className="w-6 h-6" />
            </button>
            <img 
